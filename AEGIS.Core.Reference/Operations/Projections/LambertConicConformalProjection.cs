...
namespace ELTE.AEGIS.Reference.Operations
{
    ...
    public abstract class LambertConicConformalProjection : CoordinateProjection
    {
        ...
        {
            // source: EPSG Guidance Note number 7, part 2, page 16

            return Math.Tan(Constants.PI / 4 - latitude / 2) / Math.Pow((1 - _ellipsoid.Eccentricity * Math.Sin(latitude)) / (1 + _ellipsoid.Eccentricity * Math.Sin(latitude)), _ellipsoid.Eccentricity / 2);
        }
        ...
        protected Double ComputeLatitude(Double t)
        {
            ...
            Double phi = Constants.PI / 2 - 2 * Math.Atan(t);
            for (Int32 i = 0; i < 10; i++)
            {
                phi = Constants.PI / 2 - 2 * Math.Atan(t * Math.Pow((1 - _ellipsoid.Eccentricity * Math.Sin(phi)) / (1 + _ellipsoid.Eccentricity * Math.Sin(phi)), _ellipsoid.Eccentricity / 2));
            }
