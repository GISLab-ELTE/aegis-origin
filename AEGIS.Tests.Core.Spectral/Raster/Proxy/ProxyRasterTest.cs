...
namespace ELTE.AEGIS.Tests.Raster.Proxy
{
    ...
    public class ProxyRasterTest
    {
        ...
        public void SetUp()
        {
            ...
            _entity.Setup(entity => entity.ReadValue(It.IsAny<Int32>(), It.IsAny<Int32>(), It.IsAny<Int32>())).Throws<ArgumentOutOfRangeException>();

            for (Int32 i = 0; i < 3; i++)
                _entity.Setup(entity => entity.ReadValue(It.IsInRange(0, 19, Range.Inclusive),
                                                            It.IsInRange(0, 14, Range.Inclusive),
                                                            i)).Returns((UInt32)i);

            _entity.Setup(entity => entity.ReadValueSequence(It.IsAny<Int32>(), It.IsAny<Int32>())).Throws<ArgumentOutOfRangeException>();
            _entity.Setup(entity => entity.ReadValueSequence(It.IsInRange(0, 19, Range.Inclusive),
                                                                It.IsInRange(0, 14, Range.Inclusive),
                                                                0, 3)).Returns(new UInt32[] { 0, 1, 2 });
            _entity.Setup(entity => entity.WriteValue(It.IsAny<Int32>(), It.IsAny<Int32>(), It.IsAny<Int32>(), It.IsAny<UInt32>())).Throws<ArgumentOutOfRangeException>();
            _entity.Setup(entity => entity.WriteValueSequence(It.IsAny<Int32>(), It.IsAny<Int32>(), It.IsAny<Int32>(), It.IsAny<UInt32[]>())).Throws<ArgumentOutOfRangeException>();
        ...
        public void ProxyRasterConstructorTest()
        {
            ...
            ProxyRaster raster = new ProxyRaster(_factory.Object, _entity.Object, null, null);
            ...
            raster = new ProxyRaster(_factory.Object, _entity.Object, SpectralRanges.RGB, null);
            ...
            raster = new ProxyRaster(_factory.Object, _entity.Object, null, mapper);
            ...
            Assert.Throws<ArgumentNullException>(() => { raster = new ProxyRaster(_factory.Object, null, null, null); });

            // test case 5: argument exception

            Assert.Throws<ArgumentException>(() => { raster = new ProxyRaster(_factory.Object, _entity.Object, new SpectralRange[] { SpectralRanges.Blue }, null); });
            Assert.Throws<ArgumentException>(() =>
            {
                _entity.Setup(entity => entity.Representation).Returns(RasterRepresentation.Floating);
                raster = new ProxyRaster(_factory.Object, _entity.Object, null, null);
            });     
        }
        ...
        public void ProxyRasterGetValueTest()
        {
            ...
            for (Int32 i = -1; i <= raster.SpectralResolution; i++)
                for (Int32 j = -1; j <= raster.NumberOfRows + 5; j += 5)
                    for (Int32 k = -1; k <= raster.NumberOfColumns + 5; k += 5)
                    {
                        try
                        {
                            Assert.AreEqual(_entity.Object.ReadValue(j, k, i), raster.GetValue(j, k, i));
                            Assert.AreEqual(_entity.Object.ReadValue(j, k, i), raster[j, k, i]);
                        }
            ...
            for (Int32 x = 1000; x < 1000 + 10 * _entity.Object.NumberOfColumns; x += 100)
                ...
                {
                    Assert.AreEqual(_entity.Object.ReadValue((x - 1000) / 10, (y - 1000) / 10, 0), raster.GetValue(new Coordinate(x, y), 0));
                    Assert.AreEqual(_entity.Object.ReadValue((x - 1000) / 10, (y - 1000) / 10, 0), raster[new Coordinate(x, y), 0]);
                }
        }
        ...
        public void ProxyRasterGetValuesTest()
        {
            ...
            for (Int32 j = 0; j < raster.NumberOfRows; j += 5)
                ...
                {
                    Assert.IsTrue(_entity.Object.ReadValueSequence(j, k, 0, 3).SequenceEqual(raster.GetValues(j, k)));
                    Assert.IsTrue(_entity.Object.ReadValueSequence(j, k, 0, 3).SequenceEqual(raster[j, k]));
                }
            ...
            for (Int32 x = 1000; x < 1000 + 10 * _entity.Object.NumberOfColumns; x += 100)
                for (Int32 y = 1000; y < 1000 + 10 * _entity.Object.NumberOfRows; y += 100)
                {
                    Assert.IsTrue(_entity.Object.ReadValueSequence((x - 1000) / 10, (y - 1000) / 10, 0, 3).SequenceEqual(raster.GetValues(new Coordinate(x, y))));
                    Assert.IsTrue(_entity.Object.ReadValueSequence((x - 1000) / 10, (y - 1000) / 10, 0, 3).SequenceEqual(raster[new Coordinate(x, y)]));
                }
        }
        ...
        public void ProxyRasterNearestValueTest()
        {
            ...
            for (Int32 i = 0; i < raster.SpectralResolution; i++)
                for (Int32 j = -1; j <= raster.NumberOfRows + 5; j += 5)
                    for (Int32 k = -1; k <= raster.NumberOfColumns + 5; k += 5)
                    {
                        Assert.AreEqual(_entity.Object.ReadValue(Math.Max(0, Math.Min(j, _entity.Object.NumberOfRows - 1)),
            ...
            for (Int32 i = 0; i < raster.SpectralResolution; i++)
                for (Int32 x = 1000; x < 1000 + 10 * _entity.Object.NumberOfColumns; x += 100)
                    for (Int32 y = 1000; y < 1000 + 10 * _entity.Object.NumberOfRows; y += 100)
                    {
                        Assert.AreEqual(_entity.Object.ReadValue(Math.Max(0, Math.Min((x - 1000) / 10, _entity.Object.NumberOfRows - 1)),
