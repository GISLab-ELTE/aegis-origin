/// <copyright file="FloatRaster32Test.cs" company="Eötvös Loránd University (ELTE)">
...
{
    [TestFixture]
    public class FloatRaster32Test
    {
        [TestCase]
        public void FloatRaster32ConstructorTest()
        {
            ...
            for (Int32 i = 0; i < 10; i++)
                for (Int32 j = 0; j < 10000; j += 2000)
                    ...
                    {
                        FloatRaster32 raster = new FloatRaster32(null, i, j, k, Enumerable.Repeat(8, i).ToArray(), null, null);
            ...
            Assert.Throws<ArgumentOutOfRangeException>(() => { FloatRaster32 raster = new FloatRaster32(null, -1, 1, 1, null, null, null); });
            Assert.Throws<ArgumentOutOfRangeException>(() => { FloatRaster32 raster = new FloatRaster32(null, 1, -1, 1, null, null, null); });
            Assert.Throws<ArgumentOutOfRangeException>(() => { FloatRaster32 raster = new FloatRaster32(null, 1, 1, -1, null, null, null); });
            Assert.Throws<ArgumentOutOfRangeException>(() => { FloatRaster32 raster = new FloatRaster32(null, 1, 1, 1, new Int32[] { -1 }, null, null); });
            Assert.Throws<ArgumentOutOfRangeException>(() => { FloatRaster32 raster = new FloatRaster32(null, 1, 1, 1, new Int32[] { 33 }, null, null); });


            // test case 3: argument exceptions

            Assert.Throws<ArgumentException>(() => { FloatRaster32 raster = new FloatRaster32(null, 1, 1, 1, new Int32[] { 8, 8 }, null, null); });
            Assert.Throws<ArgumentException>(() => { FloatRaster32 raster = new FloatRaster32(null, 1, 1, 1, null, SpectralRanges.RGB, null); });
        }

        [TestCase]
        public void FloatRaster32ValueTest()
        {
            FloatRaster32 raster = new FloatRaster32(null, 3, 9, 27, null, SpectralRanges.RGB, null);
            ...
            for (Int32 i = 0; i < raster.SpectralResolution; i++)
                for (Int32 j = 0; j < raster.NumberOfRows; j++)
                    for (Int32 k = 0; k < raster.NumberOfColumns; k++)
                    {
                        ...
                        Assert.AreEqual(1.0 / (i * j * k), raster.GetValue(j, k, i), 0.00001);
                    }
            ...
            for (Int32 j = 0; j < raster.NumberOfRows; j++)
                for (Int32 k = 0; k < raster.NumberOfColumns; k++)
                {
                    ...
                    Assert.AreEqual(1.0 / j, raster.GetValues(j, k)[0], 0.00001);
                    Assert.AreEqual(1.0 / k, raster.GetValues(j, k)[1], 0.00001);
                    Assert.AreEqual(1.0 / (j * k), raster.GetValues(j, k)[2], 0.00001);
                }
