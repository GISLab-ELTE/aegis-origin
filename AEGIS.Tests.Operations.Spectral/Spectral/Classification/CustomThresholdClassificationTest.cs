/// <copyright file="ThresholdTransformationTest.cs" company="Eötvös Loránd University (ELTE)">
...
{
    [TestFixture]
    public class CustomThresholdClassificationTest
    {
        ...
        public void SpectralInversionExecuteTest()
        {
            ...
            Assert.AreEqual(RasterRepresentation.Integer, operation.Result.Raster.Representation);
            ...
            for (Int32 i = 0; i < operation.Result.Raster.SpectralResolution; i++)
                for (Int32 j = 0; j < operation.Result.Raster.NumberOfRows; j += 5)
                    ...
                    {
                        Assert.AreEqual(_raster.Object.GetValue(j, k, i) >= 100 && _raster.Object.GetValue(j, k, i) <= 150 ? 255 : 0, operation.Result.Raster.GetValue(j, k, i), 0);
                    }
            ...
            for (Int32 i = 0; i < operation.Result.Raster.SpectralResolution; i++)
                for (Int32 j = 0; j < operation.Result.Raster.NumberOfRows; j += 5)
                    for (Int32 k = 0; k < operation.Result.Raster.NumberOfColumns; k += 5)
                    {
                        Assert.AreEqual(_floatRaster.Object.GetValue(j, k, i) >= 100 && _floatRaster.Object.GetValue(j, k, i) <= 150 ? 255 : 0, operation.Result.Raster.GetValue(j, k, i), 0);
                    }
        }
        ...
        public void SpectralInversionExecuteForBandTest()
        {
            ...
            for (Int32 j = 0; j < operation.Result.Raster.NumberOfRows; j += 5)
                for (Int32 k = 0; k < operation.Result.Raster.NumberOfColumns; k += 5)
                {
                    Assert.AreEqual(_raster.Object.GetValue(j, k, 0) >= 100 && _raster.Object.GetValue(j, k, 0) <= 150 ? 255 : 0, operation.Result.Raster.GetValue(j, k, 0), 0);
                }
